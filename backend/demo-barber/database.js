...
const dbPath = path.resolve(dataDir, 'demo-barber.sqlite');
const db = new sqlite3.Database(dbPath, (err) => {
    if (err) {
        console.error('Error connecting to Demo-Barber SQLite database:', err.message);
    } else {
        console.log('✅ Connected to Demo-Barber SQLite database.');
        ...
        `, () => {
            db.get("SELECT COUNT(*) as cnt FROM admins", [], (err, row) => {
                if (row && row.cnt === 0) {
                    ...
                    console.log('✅ Demo-Barber SQLite: Default admin created');
                }
            });
        });
        ...
        `);
        // Migration: add staff_id to existing bookings table
        ...
        `, () => {
            db.get("SELECT COUNT(*) as cnt FROM staff", [], (err, row) => {
                if (row && row.cnt === 0) {
                    ...
                    stmt.run('Kirpėjas A', '[1,2,3,4,5,6]', '09:00', '18:30', '[]', '[]', 1);
                    stmt.run('Kirpėjas B', '[1,2,3,4,5]', '10:00', '19:00', '[]', '[]', 2);
                    stmt.finalize();
                    console.log('✅ Demo-Barber SQLite: Default staff created');
                }
            });
        });
    }

document.addEventListener('DOMContentLoaded', () => {
    ...
    if (bDate && bTime) {
        ...
        const fetchTimes = async () => {
            ...
            try {
                const response = await fetch(`/api/demo-nails/available-times?date=${selectedDate}&service=${serviceName}`);
        ...
        async function loadMonthAvailability(year, month, instance) {
            try {
                const res = await fetch(`/api/demo-nails/availability-month?year=${year}&month=${month}`);
    ...
    if (bookingForm) {
        bookingForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('/api/demo-nails/reservations', {
                    method: 'POST',
